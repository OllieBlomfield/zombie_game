...
func _process(delta: float) -> void:
	...
	var screen_coords: Vector2 = (target_global_position - camera_node.global_position) * camera_node.zoom + viewport_dimensions * 0.5
	...
	if screen_inset_rectangle.has_point(screen_coords):
		target_display_position = target_global_position + on_screen_offset
		target_display_rotation = 0.0
	else:
		var clamped_x = clamp(screen_coords.x, screen_margin, viewport_dimensions.x - screen_margin)
		var clamped_y = clamp(screen_coords.y, screen_margin, viewport_dimensions.y - screen_margin)
